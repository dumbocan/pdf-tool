...
import { extractInvoiceEvidence, produceInvoiceEvidence } from "../src/invoice-evidence.js";
...
const fields = (record) => [record.supplier, record.invoiceNumber, record.invoiceDate, record.currency, record.taxableBase, record.taxes, record.total, ...record.lineItems.flatMap((row) => [row.description, row.quantity, row.unitPrice])];
...
test("produces a closed untrusted invoice evidence record for one digital PDF", async () => {
  ...
  assert.deepEqual(evidence.record.lineItems.map(({ description, quantity, unitPrice }) => [description.value, quantity.value, unitPrice.value]), [["Notebook A5", "4", "2.5"], ["Pen Black", "8", "1.2"], ["Stapler Heavy", "2", "8.75"]]);
  assert.equal(evidence.table.columns.length, 3);
  assert.ok(fields(evidence.record).every((field) => field.state === "PRESENT"));
  assert.ok(fields(evidence.record).every((field) => field.evidence.length >= 1));
...
test("exports the producer alias without changing the evidence contract", async () => {
  const evidence = await produceInvoiceEvidence(PDF, { documentId: DOCUMENT_ID });
  assert.equal(evidence.documentSha256.length, 64);
  assert.deepEqual(evidence.record.lineItems.map((row) => row.rowId), ["g_0000000000000000", "g_0000000000000001", "g_0000000000000002"]);
});

test("returns explicit review state and missing envelopes when required invoice data is absent", async () => {
  const evidence = await extractInvoiceEvidence(readFileSync(new URL("./fixtures/ocr-placeholder-image.pdf", import.meta.url)), { documentId: DOCUMENT_ID });
...
test("rejects an invalid document identity before PDF processing", async () => {
  await assert.rejects(() => extractInvoiceEvidence(PDF, { documentId: "not-a-document-id" }), /document id/i);
});
