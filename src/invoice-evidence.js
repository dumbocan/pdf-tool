...
export function normalizeOcrRect(bbox, pageWidth, pageHeight) {
  ...
  const bottom = Math.max(0, Math.min(pageHeight, bbox.y));
  const right = Math.max(left, Math.min(pageWidth, bbox.x + bbox.width));
  const top = Math.max(0, Math.min(pageHeight, pageHeight - bbox.y - bbox.height));
...
export function materializeOcrEvidence(tokens, { pageWidth, pageHeight } = {}) {
  ...
  if (tokens.length > MAX_OCR_EVIDENCE_FRAGMENTS) throw ocrGeometryError("resource_limit");
  const normalized = tokens.map((token, sequence) => {
    if (token?.tokenId !== undefined && !TOKEN_ID_RE.test(token.tokenId)) throw ocrGeometryError("token_id_invalid");
  ...
  const fragments = [];
  const lines = [];
  for (const [page, pageTokens] of [...byPage.entries()].sort(([a], [b]) => a - b)) {
    const threshold = Math.max(1, median(pageTokens.map(({ rect }) => rect.height)) * 1.5);
    const ordered = [...pageTokens].sort((a, b) => a.rect.y - b.rect.y || a.rect.x - b.rect.x || a.sequence - b.sequence);
    const groups = [];
    ...
    for (const [row, group] of groups.entries()) {
      ...
      const maxX = Math.max(...group.tokens.map(({ rect }) => rect.x + rect.width));
      const maxY = Math.max(...group.tokens.map(({ rect }) => rect.y + rect.height));
      lines.push({
        text: group.tokens.map(({ token }) => token.text.trim()).join(" "),
        pageNumber: page,
        bbox: { page, x: minX / 100, y: minY / 100, width: (maxX - minX) / 100, height: (maxY - minY) / 100 },
        ocrFragments: groupFragments,
            // Internal parser seam; token text never crosses the evidence wire.
            ocrTokens: group.tokens.map(({ token, fragment, rect, column, row }) => ({
              ...token,
              fragment,
              rect,
              column,
  ...
  return { fragments, lines, tokens: normalized, pageCount: byPage.size ? Math.max(...byPage.keys()) : 1 };
      ...
      if (!Array.isArray(tokens) || !Number.isFinite(pageWidth) || !Number.isFinite(pageHeight)) {
...
function makeRow(description, quantity, unitPrice, source, ordinal, cellFragments) {
  ...
  const fragments = cellFragments ?? fallbackFragments.map((fragment) => fragment ? [fragment] : []);
  const page = fragments.flat().find(Boolean)?.page ?? source?.pageNumber ?? 1;
  const envelope = (value, evidence) => evidence?.length ? PRESENT(value, evidence) : MISSING("EVIDENCE_MISSING");
  return {
    rowId: ID("g", ordinal), page, ordinal,
...
function ocrHeaderInfo(text) {
  ...
  const isHeader = has("description") && (has("qty") || has("quantity")) &&
has("unit") && has("price") && (has("amount") || has("total"));
  return { isHeader, ambiguous: false };
}

function parseRows(lines) {
  const rows = [], seen = new Set(), headers = [], issues = [];
  const rowPattern = new RegExp(`^(.+?)\\s+(${ROW_NUMBER})\\s+(${ROW_NUMBER})\\s+${ROW_NUMBER}$`);
  for (let index = 0; index < lines.length; index += 1) {
    const source = lines[index];
    const text = source.text.trim();
    const header = ocrHeaderInfo(text);
    const headerWords = OCR_HEADER_WORDS.filter((word) => text.toLowerCase().includes(word)).length;
    if (headerWords >= 2) {
      if (header.isHeader) headers.push(source);
      if (header.ambiguous || !header.isHeader) issues.push("UNSUPPORTED_STRUCTURE");
      continue;
    }
    if (!text || /^(invoice|invoice number|invoice date|currency|taxable base|tax(?:es)?|total)\b/i.test(text)) continue;
    const match = text.match(rowPattern);
    if (!match) {
      const next = lines[index + 1]?.text?.trim() ?? "";
      if (/^.+\s+$/.test(text) || /\d/.test(text) || (text && new RegExp(`^${ROW_NUMBER}(?:\\s+${ROW_NUMBER})+$`).test(next))) issues.push("UNSUPPORTED_STRUCTURE");
      continue;
    }
    const description = match[1].trim();
    const quantity = canonicalNumber(match[2], 6);
    const unitPrice = canonicalNumber(match[3], 4);
    if (!description || !quantity || !unitPrice) { issues.push("INVALID_FORMAT"); continue; }
...
function ocrHeaderColumns(header) {
  ...
  const find = (patterns) => tokens.find(({ text = "" }) => patterns.includes(text.trim().toLowerCase()));
  const anchors = [find(["qty", "quantity"]), find(["unit", "unit price"]), find(["amount", "line", "line total"])]
...
function assignOcrNumericColumns(numeric, headerColumns) {
  if (!headerColumns || numeric.length === 0 || numeric.length > headerColumns.length) return null;
  const boundaries = [
    -Infinity,
  ...
  const assignments = numeric.map((entry) => {
    ...
    const column = boundaries.findIndex((boundary, index) => x >= boundary && x < boundaries[index + 1]);
    return { entry, column };
  });
  if (assignments.some(({ column }) => column < 0) || assignments.some(({ column }, index) => index && column <= assignments[index - 1].column)) return null;
  return assignments;
}

function ocrRowCandidate(source, headerColumns) {
  const tokens = Array.isArray(source.ocrTokens) ? source.ocrTokens : [];
  const numeric = tokens.filter(({ text = "" }) => new RegExp(`^${ROW_NUMBER}$`).test(text.trim()));
  ...
  fragments[0] = descriptionTokens.flatMap(({ fragment }) => fragment ? [fragment] : []);
  return {
...
function isOcrDescriptionOnly(source) {
  if (!Array.isArray(source?.ocrTokens) || source.ocrTokens.length === 0) return false;
  return !source.ocrTokens.some(({ text = "" }) => new RegExp(`^${ROW_NUMBER}$`).test(text.trim()));
}
...
function parseOcrRows(lines) {
  ...
  for (const source of clusteredLines) {
    ...
    const headerWords = OCR_HEADER_WORDS.filter((word) => source.text.toLowerCase().includes(word)).length;
    const numeric = (source.ocrTokens ?? []).filter(({ text = "" }) => new RegExp(`^${ROW_NUMBER}$`).test(text.trim()));
    if (headerWords >= 2) {
      if (header.isHeader && numeric.length === 0) headers.push(source);
      if (header.ambiguous || !header.isHeader || numeric.length > 0) issues.push("UNSUPPORTED_STRUCTURE");
    }
  }
  ...
  for (let index = 0; index < clusteredLines.length; index += 1) {
    ...
    if (!text || /^(invoice|invoice number|invoice date|currency|taxable base|subtotal|tax(?:es)?|total)\b/i.test(text)) continue;
    const numeric = (source.ocrTokens ?? []).filter(({ text: value = "" }) => new RegExp(`^${ROW_NUMBER}$`).test(value.trim()));
    ...
    const wrapped = previous && previous.pageNumber === source.pageNumber && isOcrDescriptionOnly(previous)
      && !ocrHeaderInfo(previous.text).isHeader
      && !/^(invoice|invoice number|currency|taxable base|subtotal|tax(?:es)?|total)\b/i.test(previous.text.trim());
    const descriptionTokens = wrapped ? [...previous.ocrTokens, ...candidate.descriptionTokens] : candidate.descriptionTokens;
    const description = descriptionTokens.map(({ text: value }) => value.trim()).join(" ");
    const cellFragments = [
    ...
    if (!description || !candidate.quantity || !candidate.unitPrice) {
      if (description && candidate.cellFragments.some((fragments, cell) => cell > 0 && fragments.length === 0)) issues.push("MISSING_EVIDENCE");
    ...
    if (seen.has(key)) continue;
    seen.add(key);
    rows.push(makeRow(description, candidate.quantity, candidate.unitPrice, source, rows.length, cellFragments));
    if (rows.length === 500) break;
  }
  return { rows, headers, issues };
}
  ...
  return { rowId: ID("g", 0), page: 1, ordinal: 0, description: MISSING(reason), quantity: MISSING(reason), unitPrice: MISSING(reason) };
}
...
function missingReasons(record, hasText, extra = []) {
  ...
  for (const [name, value] of Object.entries(record)) {
    ...
    if (value.state === "MISSING") reasons.push({ EVIDENCE_MISSING: "MISSING_EVIDENCE", INVALID_FORMAT: "INVALID_FORMAT", UNSUPPORTED: "CURRENCY_INVALID", AMBIGUOUS: "AMBIGUOUS_VALUE" }[value.reason] ?? "MISSING_REQUIRED_VALUE");
  }
  for (const row of record.lineItems) for (const value of [row.description, row.quantity, row.unitPrice]) {
...
function fieldEnvelope(value, source, ordinal, fallbackReason = "NOT_FOUND") {
  ...
  return fragments.length ? PRESENT(value, fragments) : MISSING("EVIDENCE_MISSING");
}
...
function validDate(value) {
  ...
  return year >= 1900 && year <= 9999 && date.getUTCFullYear() === year && date.getUTCMonth() === month - 1 && date.getUTCDate() === day;
}

function addDecimal(a, b) {
  const [aw, af = ""] = a.split("."), [bw, bf = ""] = b.split("."), scale = Math.max(af.length, bf.length);
  const sum = (BigInt(aw + af.padEnd(scale, "0")) + BigInt(bw + bf.padEnd(scale, "0"))).toString().padStart(scale + 1, "0");
  return scale ? `${sum.slice(0, -scale)}.${sum.slice(-scale).replace(/0+$/, "") || "0"}` : sum;
}
...
function unavailableOcrEvidence(documentId, documentSha256) {
  return {
    invoiceEvidenceSchemaVersion: "1", documentId, documentSha256, extractionMode: "OCR_REQUIRED_UNAVAILABLE",
    pageCount: 1, extractedCharacterCount: 0,
...
function ocrConfidenceBps(tokens) {
   ...
   const weighted = tokens.map(({ token, rect }) => {
     ...
     const confidence = Number.isFinite(raw) ? Math.max(0, Math.min(10000, raw > 100 ? Math.round(raw) : Math.round(raw * 100))) : 0;
...
export async function extractInvoiceEvidence(buffer, { documentId, digitalExtractor, ocrExtractor, ocrOptions, scalarLabelsExtension } = {}) {
  ...
  if (text.trim().length === 0) {
    ...
    ocrTokens = Array.isArray(ocr?.tokens) ? ocr.tokens : [];
        ...
        try {
      const materialized = materializeOcrEvidence(ocrTokens, { pageWidth: ocr?.pageWidth, pageHeight: ocr?.pageHeight });
  ...
  const supplierCandidate = supplierName && supplierFragments.length ? { supplierCandidateId: supplierId, displayName: supplierName, evidence: supplierFragments } : null;
    ...
    return fieldEnvelope(raw == null ? null : normalizer(raw), source, ordinal, fallback);
  };
  const dateSource = lineSource(lines, /invoice\s+date\s*:/i);
  const dateRaw = dateSource?.text.match(/invoice\s+date\s*:\s*(\S+)/i)?.[1] ?? null;
  ...
  if (extractionMode === "OCR" && ocrTokens.length === 0) semanticReasons.push("MISSING_EVIDENCE");
  if ([record.taxableBase, record.taxes, record.total].every((value) => value.state === "PRESENT") && addDecimal(record.taxableBase.value, record.taxes.value) !== record.total.value) semanticReasons.push("ARITHMETIC_INVALID");
  const reasons = missingReasons(record, text.trim().length > 0, semanticReasons);
  ...
  const unsupported = reasons.some((reason) => ["CREDIT_NOTE", "UNSUPPORTED_STRUCTURE"].includes(reason));
  return {
    invoiceEvidenceSchemaVersion: "1", documentId, documentSha256, extractionMode,
    pageCount: Math.max(1, Math.min(100, pageCount)), extractedCharacterCount: Math.min(80_000, text.length),
    iso4217Snapshot: { version: ISO_SNAPSHOT.version, checksumSha256: ISO_SNAPSHOT.checksumSha256 }, supplierCandidate, record,
    table: analysis.table, confidenceBps: ocrConfidence ?? Math.round((presentCount / valueCount) * 10_000),
    recordOutcome: extractionMode === "OCR"
